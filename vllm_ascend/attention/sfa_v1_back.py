...
from vllm_ascend.distributed.sfa_sp_context import (set_sfa_sp_context, get_sfa_sp_context,
                                                get_sfa_tmp_result_context, set_sfa_tmp_result_context)
...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def forward(
        ...
        if self.enable_sfa_cp:
            if pad_size > 0:
                cos = nn.functional.pad(cos,
                                                  (0, 0, 0, 0, 0, 0, 0, pad_size))
                sin = nn.functional.pad(sin,
                                                  (0, 0, 0, 0, 0, 0, 0, pad_size))
                cos = cos[start_index:end_index]
                sin = sin[start_index:end_index]

                slot_mapping_cp = nn.functional.pad(slot_mapping,
                                                  (0, pad_size))
                slot_mapping_cp = slot_mapping_cp[start_index:end_index]
        new_q_pe = self.rope_single(new_q_pe, cos, sin)
        new_k_pe, new_k_nope = self.exec_kv_new(new_kv_no_split, cos,
                                    sin, kv_cache, slot_mapping_cp, attn_metadata)
        #new_ql_nope = get_tp_group().all_gather(new_ql_nope, 0)
        #new_q_pe = get_tp_group().all_gather(new_q_pe, 0)
        #new_k_pe = get_tp_group().all_gather(new_k_pe, 0)
        #new_k_nope = get_tp_group().all_gather(new_k_nope, 0)
        ...
        attn_output_old = get_tp_group().all_gather(attn_output, 1)
    ...
    def indexer_select_new(
        ...
        topk_indices = torch.ops.custom.npu_lightning_indexer(
            ...
            weights=weights,
            actual_seq_lengths_query=cum_query_lens_cp,
            actual_seq_lengths_key=seq_lens_cp,
            block_table=block_table,
            layout_query="TND",
            layout_key="PA_BSND",
            sparse_count=2048,
