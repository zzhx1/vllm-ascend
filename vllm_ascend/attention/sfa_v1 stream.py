...
sfa_cp_stream: Optional[torch.npu.Stream] = None
...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.enable_sfa_cp:
            ...
            if sfa_cp_stream is None:
                sfa_cp_stream = torch.npu.Stream()
    ...
    def exec_kv(
        ...
        if self.enable_sfa_cp:
            ...
            return torch.cat([k_pe.view(-1, k_pe.shape[-1]), k_nope.view(-1, k_nope.shape[-1])], dim=1)
        else:
    ...
    def forward(
        ...
        if self.enable_mlapo and not forward_context.with_prefill:
            ...
            q, k = self.indexer_select_pre_process(
                ...
                need_gather_q_kv=need_gather_q_kv)
            if self.enable_sfa_cp:
                assert sfa_cp_stream is not None
                sfa_cp_stream.wait_stream(torch.npu.current_stream())
                with npu_stream_switch(sfa_cp_stream, enabled=self.enable_sfa_cp):
            ...
            k_pe_k_nope_no_split = self.exec_kv(kv_no_split, cos, sin, kv_cache, slot_mapping, slot_mapping_cp)
            if self.enable_sfa_cp:
                assert k_pe_k_nope_no_split is not None
                assert sfa_cp_stream is not None
                # Make sure all_gather k is done.
                torch.npu.current_stream().wait_stream(sfa_cp_stream)
                sfa_cp_stream.wait_stream(torch.npu.current_stream())
                with npu_stream_switch(sfa_cp_stream, enabled=self.enable_sfa_cp):
                    k_pe, k_nope = get_tp_group().all_gather(k_pe_k_nope_no_split, 0).split([self.qk_rope_head_dim, self.kv_lora_rank], dim=-1)
                    if is_hidden_layer(self.vllm_config, self.q_proj):
                        reach_layer_for_shared_weight_series(self.q_proj)
                    if is_hidden_layer(self.vllm_config, self.o_proj):
                        reach_layer_for_shared_weight_series(self.o_proj)
