...
#ifndef FUSED_SPARSE_ATTENTION_OVERLAP_KERNEL_MLA_ARCH35_H
#define FUSED_SPARSE_ATTENTION_OVERLAP_KERNEL_MLA_ARCH35_H
...
#include "fused_sparse_attention_overlap_kvcache.h"
...
namespace BaseApi {
template <typename CubeBlockType, typename VecBlockType> class FusedSparseAttentionOverlapKernelMla {
public:
    ARGS_TRAITS;

    __aicore__ inline FusedSparseAttentionOverlapKernelMla(){};
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                ...
                                __gm__ uint8_t *selectionMembershipMap, __gm__ uint8_t *attentionOut,
                                __gm__ uint8_t *selectionKvActualSeq, __gm__ uint8_t *workspace,
...
private:
    ...
    /* Maximum s2Loop count when G is split */
    int64_t maxS2LoopCnt;

    /* Information that remains constant after initialization */
    ConstInfo constInfo;
...
template <typename CubeBlockType, typename VecBlockType>
 __aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::Init(
    ...
    __gm__ uint8_t *workspace, const FusedSparseAttentionOverlapTilingDataMla *__restrict tiling,
    __gm__ uint8_t *gmTiling, TPipe *tPipe)
{
    ...
    vecBlock.InitVecBlock(tPipe, this->tilingData, this->sharedParams,
        constInfo.subBlockIdx, actualSeqLengthsQ, actualSeqLengths);
    if ASCEND_IS_AIV {
        ...
        constInfo.returnSoftmaxLse = false;
    }
    vecBlock.CleanOutput(attentionOut, constInfo);
    /* Cube-side scalar setup that does not depend on sharedParams */
...
#pragma unroll
    ...
    this->InitGlobalBuffer(query, key, value, queryRope, keyRope, sparseIndices, \
        blockTable, actualSeqLengthsQ, actualSeqLengths, selectionKRope, selectionKvCache, \
        selectionKvBlockTable, selectionKvBlockStatus, selectionMembershipMap, selectionKvActualSeq, \
        workspace, tiling, tPipe); // Configure GM
    this->InitCalcParamsEach();
    this->InitLocalBuffer();
}

template <typename CubeBlockType, typename VecBlockType> __aicore__ inline
void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::InitCalcParamsEach()
{
    // Calculate the total number of base blocks.
    maxS2LoopCnt = 0; // Maximum accumulated s2Loop count across all cores
    uint32_t totalBaseNum = 0;
    uint32_t s1GBaseSize = constInfo.gSize;
    uint32_t actBatchS2 = 1;
    uint32_t coreNum = GetBlockNum(); // For G=128, two adjacent Cube cores process one S1, so halve coreNum
    ...
    for (uint32_t bIdx = 0; bIdx < constInfo.bSize; bIdx++) {
        actBatchS1 = GetBalanceActualSeqLengths(actualSeqLengthsQGm, bIdx); // S2 is not split; only S1 is considered
...
FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::GetBalanceActualSeqLengths(
...
template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::GetAxisStartIdx(uint32_t bN2EndPrev,
                                                                                uint32_t s1GEndPrev,
                                                                                uint32_t s2EndPrev)
{
...
template <typename CubeBlockType, typename VecBlockType> __aicore__ inline
void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::InitGlobalBuffer(
    __gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
    __gm__ uint8_t *queryRope, __gm__ uint8_t *keyRope, __gm__ uint8_t *sparseIndices,
    __gm__ uint8_t *blockTable, __gm__ uint8_t *actualSeqLengthsQ, __gm__ uint8_t *actualSeqLengths,
    __gm__ uint8_t *selectionKRope, __gm__ uint8_t *selectionKvCache,
    __gm__ uint8_t *selectionKvBlockTable, __gm__ uint8_t *selectionKvBlockStatus,
    __gm__ uint8_t *selectionMembershipMap, __gm__ uint8_t *selectionKvActualSeq,
    __gm__ uint8_t *workspace, const FusedSparseAttentionOverlapTilingDataMla *__restrict tiling, TPipe *tPipe)
{
    ...
    vecBlock.InitGlobalBuffer(key, value, keyRope, sparseIndices, blockTable,
        selectionKRope, selectionKvCache, selectionKvBlockTable, selectionKvBlockStatus,
        selectionMembershipMap, selectionKvActualSeq);
    cubeBlock.InitCubeInput(key, keyRope, sparseIndices, blockTable, actualSeqLengthsQ, constInfo);
}

template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void
FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::InitMMResBuf(__gm__ uint8_t *workspace)
{
    ...
    // The L1 memory holding P results must use the first L1 policy so its address matches the Vector allocation.
...
template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::InitLocalBuffer()
{
    vecBlock.InitLocalBuffer(pipe, constInfo);
}

template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::ComputeConstexpr()
{
    ...
    } else if constexpr (LAYOUT_T == FusedSparseAttentionOverlapLayoutArch35::BSND) {
        // BSH/BSNGD
        constInfo.s1BaseN2GDv = constInfo.s1BaseSize * constInfo.n2GDv;
        constInfo.mm1Ka = constInfo.n2Size * constInfo.dSize;
        if ASCEND_IS_AIV {
...
template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::InitUniqueConstInfo()
{
    // bsize + 1-> bsize
...
template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::Process()
{
    // Both Cube and Vector must call SyncAll.
...
template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::ProcessMainLoop()
{
        ...
        if ASCEND_IS_AIV {
            if constexpr (IS_SPLIT_G) {
                for (int64_t loopCnt = 0; loopCnt < maxS2LoopCnt; loopCnt++) {
                    CrossCoreSetFlag<FSA_ARCH35_SYNC_MODE0, PIPE_MTE3>(15);
                    CrossCoreWaitFlag<FSA_ARCH35_SYNC_MODE0, PIPE_MTE3>(15);
                }
            }
        }
        return;
    ...
    // Adapt core partitioning to a left-closed, right-open interval.
    ...
    if (constInfo.gS1End + 1 < gS1max) {
        /* When constInfo.gS1End != gS1max, increment gS1End by one and leave bN2End unchanged. */
        constInfo.gS1End = constInfo.gS1End + 1;
    } else {
        /* When constInfo.gS1End == gS1max, advance bN2End and set gS1End to 0 to represent the end. */
    ...
    for (int64_t bnIdx = bN2StartIdx; bnIdx < bN2EndIdx; bnIdx++) {
        ...
        for (int64_t gS1Index = runParam.gs1LoopStartIdx; gS1Index < gS1LoopEnd; gS1Index++) {
            ...
            if (notLastTwoLoop) {
                ...
                // Continue when either S1 or S2 requires no computation. On the current core's final iteration,
                // also complete the taskIdx + 2 work.
                bool s2NoNeedCalc =
...
template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::ComputeAxisIdxByBnAndGs1(
    int64_t bnIndex, int64_t gS1Index, RunParamStr &runParam)
{
    // Merge the G and S1 axes; do not split G, only split S1.
    runParam.s1oIdx = gS1Index * runParam.qSNumInOneBlock;
    if constexpr (IS_SPLIT_G) {
        runParam.goIdx = (aicIdx % 2 == 0) ? 0 : 64; // For N1=128, adjacent Cube cores process one S1: the first handles G rows 0-63 and the second handles the remaining 64 rows
    } else {
        runParam.goIdx = 0;
    }
}
...
template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::SetRunInfo(
...
template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::InitUniqueRunInfo(
...
template <typename CubeBlockType, typename VecBlockType>
__aicore__ inline void FusedSparseAttentionOverlapKernelMla<CubeBlockType, VecBlockType>::ComputeBmm1Tail(
...
#endif // FUSED_SPARSE_ATTENTION_OVERLAP_KERNEL_MLA_ARCH35_H
