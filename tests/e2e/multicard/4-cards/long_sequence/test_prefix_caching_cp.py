...
MODELS = ["vllm-ascend/Qwen3-30B-A3B-W8A8", "vllm-ascend/DeepSeek-V2-Lite-W8A8"]
...
def test_models_prefix_cache_with_cp_basic(model: str, max_tokens: int, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with VllmRunner(
        ...
        prefix_cache_outputs = vllm_model.generate_greedy(INPUT_PROMPTS, max_tokens)
...
@pytest.mark.parametrize("model", MODELS)
@pytest.mark.parametrize("max_tokens", [2])
def test_models_prefix_cache_with_cp_full_graph(model: str, max_tokens: int, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("HCCL_BUFFSIZE", "768")
    with VllmRunner(
        model,
        block_size=128,
